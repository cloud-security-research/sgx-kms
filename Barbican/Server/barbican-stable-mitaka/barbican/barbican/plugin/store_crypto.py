...
class StoreCryptoAdapterPlugin(object):
    ...
    def store_secret(self, secret_dto, context):
        ...
        enc_keys = _get_enc_keys(context.project_model.external_id)
        encrypt_dto = None
        if enc_keys:
            encrypt_dto = crypto.EncryptDTO(secret_bytes, enc_keys)
        else:
    ...
    def get_secret(self, secret_type, metadata, context):
        ...
        decrypt_dto = None
        if enc_keys:
            decrypt_dto = crypto.DecryptDTO(encrypted, enc_keys)
        else:
...
def _get_enc_keys(project_id):
    sk_repo = repositories.get_encryption_key_repository()
    sk = sk_repo.get_by_external_project_id(project_id,
                                                 suppress_exception=True)
    if sk:
