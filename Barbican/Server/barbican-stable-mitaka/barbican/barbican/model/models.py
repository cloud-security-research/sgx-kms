...
class Project(BASE, SoftDeleteMixIn, ModelBase):
    ...
    project_session_key = orm.relationship("EncryptionKey", backref="project")
    project_policies = orm.relationship("ProjectPolicy", backref="project")

    def _do_extra_dict_fields(self):
        """Sub-class hook method: return dict of fields."""
        return {'external_id': self.external_id}

class EncryptionKey(BASE, SoftDeleteMixIn, ModelBase):
    """Represent session key of a particular project.

    Session keys are encryption keys with which secure channel between user
    and barbicna server is established.
    """

    __tablename__ = 'encryption_keys'

    session_key = sa.Column(sa.Text)
    master_key = sa.Column(sa.Text)
    ...
    def __init__(self, project_id=None, session_key=None, master_key=None):
        super(EncryptionKey, self).__init__()
        self.project_id = project_id
        self.session_key = session_key
        self.master_key = master_key

class ProjectPolicy(BASE, SoftDeleteMixIn, ModelBase):

    __tablename__ = 'project_policies'

    mr_s = sa.Column(sa.Text)
    mr_e = sa.Column(sa.Text)
    mr_e_list = sa.Column(sa.Text)
    ...
    def __init__(self, project_id=None, policy=None, mr_s=None, mr_e=None, mr_e_list=None):
        super(ProjectPolicy, self).__init__()
        self.project_id = project_id
        self.policy = policy
        self.mr_s = mr_s
        self.mr_e = mr_e
        self.mr_e_list = mr_e_list
