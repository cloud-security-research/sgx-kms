...
_ENCRYPTION_KEY_REPOSITORY = None
_PROJECT_POLICY_REPOSITORY = None
...
class EncryptionKeyRepo(BaseRepo):
    """Repository for the EncryptionKey entity

    Stores encryption keys information of project.
    """
    ...
    def _do_build_get_query(self, entity_id, external_project_id, session):
        """Sub-class hook: build a retrieve query."""
        return session.query(models.EncryptionKey).filter_by(id=entity_id)
    ...
    def get_by_external_project_id(self, external_project_id,
                                   suppress_exception=False, session=None):
        session = self.get_session(session)
        query = session.query(models.EncryptionKey)
        query = query.join(models.Project, models.EncryptionKey.project)
        query = query.filter(models.Project.external_id == external_project_id)
        try:
            entity = query.one()
        except sa_orm.exc.NoResultFound:
    ...
    def create_or_update_by_project_id(self, project_id,
        ...
        try:
            ...
            self.create_from(
                models.EncryptionKey(project_id, session_key, master_key),
                    session=session)
        else:
            self._update_values(entity, {'session_key' : session_key, 'master_key' : master_key})
            entity.save(session)

    def delete_by_project_id(self, project_id, session=None):
        session = self.get_session(session)
        query = session.query(models.EncryptionKey)
        query = query.filter_by(project_id=project_id)
        query.delete()

class ProjectPolicyRepo(BaseRepo):
    """Repository for the EncryptedDatum entity
    ...
    def _do_build_get_query(self, entity_id, external_project_id, session):
        """Sub-class hook: build a retrieve query."""
        return session.query(models.ProjectPolicy).filter_by(id=entity_id)
    ...
    def get_by_external_project_id(self, external_project_id,
        ...
        query = query.join(models.Project, models.ProjectPolicy.project)
        query = query.filter(models.Project.external_id == external_project_id)
        try:
            entity = query.one()
        except sa_orm.exc.NoResultFound:
...
def get_encryption_key_repository():
    """Returns a singleton Session Key repository instance."""
    global _ENCRYPTION_KEY_REPOSITORY
    return _get_repository(_ENCRYPTION_KEY_REPOSITORY, EncryptionKeyRepo)

def get_project_policy_repository():
    """Returns a singleton Project Policy repository instance."""
    global _PROJECT_POLICY_REPOSITORY
    return _get_repository(_PROJECT_POLICY_REPOSITORY, ProjectPolicyRepo)
